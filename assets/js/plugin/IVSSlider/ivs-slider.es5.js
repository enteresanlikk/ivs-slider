...
var IVSSlider = function IVSSlider(elStr, args) {
...
function slide() {
  ...
  var elStr = arguments.length > 2 ? arguments[2] : undefined;
...
function setIVSElems(elStr) {
  var el = document.querySelector(elStr);
  var slideObj = sliderObject[elStr]; //Slider özelleştirilmeden önce içerik kopyalanıyor
  ...
  el.classList.add(DOM.slider); //Slider için gerekli olan kısımlar ekleniyor
...
function setSlide(index, elStr) {
  ...
  for (var i = index * slideObj.slidesPerView; i < index * slideObj.slidesPerView + slideObj.slidesPerView; i++) {
    i;
...
function setSlideAction(index, elStr) {
  ...
  var px = slideObj.spaceBetween; // console.log('kalan slide sayısı', Math.ceil(slideObj.slider.allCount%slideObj.slidesPerView));
...
function getIndex(elems, elem) {
  return _toConsumableArray(elems).indexOf(elem);
}